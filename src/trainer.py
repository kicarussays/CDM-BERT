...
class EHRTrainer:
    ...
    def train(self, gpu=None, ngpus_per_node=None, args=None):
        ...
        if args.external != args.hosp:
            print('Use external pretrained file.')
            params = f'm{self.args.masked_ratio}_l{self.args.use_label_type}'
            ext_model_pth = f'../results/pretrain_{self.args.external}_{self.args.external}/saved/{params}/pretrain_ckpt_seed100.tar'
            checkpoint = torch.load(ext_model_pth, map_location=f'cuda:{self.device}')
    ...
    def forward_pass(self, batch: dict):
        ...
        model_input = {
            ...
            'label_type_ids': batch['label'] if self.args.use_label_type else None,
...
class EHRClassifier(EHRTrainer):
    ...
    def train(self):
        ...
        self.valid_dl = DataLoader(self.valid_dataset, batch_size=self.args.bs, shuffle=False)
        self.test_dl = DataLoader(self.test_dataset, batch_size=self.args.bs, shuffle=False)
